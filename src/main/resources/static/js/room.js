...
let ws = null;
let wsIntentClose = false;  // 나가기 등 의도적 종료면 재연결 안 함
let reconnectTimer = null;  // 끊긴 연결 자동 복구 타이머
let reconnecting = false;   // 재연결 중 알림 1회만 뜨게
let isHost = false;
let isAiRoom = false;
let myCode = null;
...
let lastQ = null;       // 직전 질문 {name,text} — 사람 출제방에서 답글 대상 추정용
...
function renderLogEntry(e) {
  ...
  else if (e.k === "verdict") appendVerdict(e.verdict, e.q, e.asker);
...
function connectWs(code) {
  ...
  ws.onopen = () => {
    if (reconnecting) { appendSystem("다시 연결됐어요 ✅"); reconnecting = false; }
    ws.send(JSON.stringify({ type: "join", nickname: me() }));  // 서버가 puzzle/solution 재전송 → 화면 자동 복원
  };
  ws.onmessage = (e) => handleEvent(JSON.parse(e.data));
  ws.onclose = () => {
    if (wsIntentClose || !myCode) return;                       // 의도적 종료(나가기)면 재연결 안 함
    if (!reconnecting) { reconnecting = true; appendSystem("연결이 끊겼어요. 다시 연결 중… 🔄"); }
    scheduleReconnect();                                        // 카톡 등 딴 앱 갔다 오면 모바일이 소켓을 끊음 → 자동 복구
  };
  ws.onerror = () => { try { ws.close(); } catch (e) {} };
}

// 끊긴 방 소켓을 잠시 뒤 다시 연결 (서버 다운/이동 중이어도 살아나면 자동 복구)
function scheduleReconnect() {
  if (reconnectTimer || wsIntentClose || !myCode) return;
  reconnectTimer = setTimeout(() => {
    reconnectTimer = null;
    if (!wsIntentClose && myCode) connectWs(myCode);
  }, 1200);
}
...
function handleEvent(ev) {
  ...
  else if (ev.type === "question") { lastQ = { name: ev.nickname, text: ev.text }; addChat({ k: "question", name: ev.nickname, text: ev.text }); }
  else if (ev.type === "answer") addChat({ k: "verdict", verdict: ev.verdict,
      q: ev.question || (lastQ && lastQ.text) || "", asker: ev.asker || (lastQ && lastQ.name) || "" });
...
function appendVerdict(v, q, asker) {
  const pill = document.createElement("span");
  pill.className = "verdict " + (VCLASS[v] || "unknown");
  pill.textContent = VERDICT[v] || v;
  if (!q) { row("bot", judgeName(), pill); return; }   // 질문 원문 없으면 예전처럼 판정만
  const wrap = document.createElement("div");
  const quote = document.createElement("div");
  quote.className = "reply-quote";
  quote.textContent = "↳ " + (asker ? asker + ": " : "") + q;   // 어떤 질문에 대한 답인지 답글로
  wrap.append(quote, pill);
  row("bot", judgeName(), wrap);
}
...
function resetRoomControls() {
  ...
  if (ri) ri.placeholder = "메시지 입력 (질문은 / 로 시작)";
}

function endGame() {
  // 게임이 끝나도 자유 채팅은 유지 — room-input/room-send는 안 잠금(질문만 sendQuestion에서 차단)
  ["hint-input", "hint-send", "ai-hint-btn", "ai-reveal-btn"].forEach(id => {
    const el = document.getElementById(id);
    if (el) el.disabled = true;
  });
  document.querySelectorAll("#host-controls button").forEach(b => b.disabled = true);
  const inp = document.getElementById("room-input");
  if (inp) inp.placeholder = "게임 종료 — 자유 채팅만 가능해요 (질문 ✕)";
}
...
function restartGame() {
  roomLog = []; roomEnded = false; hintShown = 0; lastQ = null;
...
function sendQuestion() {
  ...
  if (raw.startsWith(ASK_PREFIX)) {
    ...
    if (roomEnded) { toast("게임이 끝나서 질문은 못 해요 — 자유 채팅만 가능해요 🐢"); return; }
...
function leaveRoom() {
  wsIntentClose = true; reconnecting = false;
  clearTimeout(reconnectTimer); reconnectTimer = null;
  if (ws) ws.close();
...
// 손으로 전송 버튼 탭할 때 input 포커스가 안 빠지게 → 모바일 키보드 안 내려감(엔터와 동일 동작)
document.getElementById("room-send").addEventListener("mousedown", e => e.preventDefault());
...
document.getElementById("hint-send").addEventListener("mousedown", e => e.preventDefault()); // 힌트 전송도 키보드 유지
...
document.addEventListener("visibilitychange", () => {
  ...
  if (dead) { clearTimeout(reconnectTimer); reconnectTimer = null; connectWs(myCode); }
});
