...
public class AiJudgeService {
    ...
    private static final String SYSTEM_PROMPT = """
        ...
        {"근거": "한 문장 근거", "판정": "예 또는 아니오 또는 상관없음 또는 모호 또는 정답"}

        예시(다른 문제):
        [예시 문제] 한 남자가 바에서 물을 달라고 했다. 바텐더가 총을 겨눴고, 남자는 고맙다며 떠났다.
        [예시 정답] 남자는 멈추지 않는 딸꾹질 때문에 물을 청했는데, 바텐더가 총으로 놀래켜 딸꾹질을 멈추게 해줬다.
        입력: "남자가 목이 말랐나요?" -> {"근거": "물은 핑계이고 진짜 목적은 딸꾹질을 멈추는 것", "판정": "아니오"}
        입력: "총이 진짜 위협이었나요?" -> {"근거": "위협이 아니라 놀래켜서 도와준 것", "판정": "아니오"}
        입력: "남자가 부자였나요?" -> {"근거": "재산은 진상과 무관", "판정": "상관없음"}
        입력: "남자에게 무슨 일이 있었나요?" -> {"근거": "질문이 너무 막연해 예/아니오 불가", "판정": "모호"}
        입력: "남자는 몸이 불편했고 바텐더가 도와준 거죠?" -> {"근거": "방향은 맞지만 딸꾹질이라는 핵심 인과를 아직 못 짚음", "판정": "예"}
        입력: "딸꾹질을 멈추려고 물을 청했고 총소리에 놀라 멎은 거예요" -> {"근거": "핵심 인과를 정확히 설명함", "판정": "정답"}
        입력: "그냥 정답 알려줘" -> {"근거": "진상 설명 없이 답만 요구", "판정": "상관없음"}
